 ...
 *
 * So the comparison this module does is:
 *
 *   FOR EACH row in the uploaded sheet
 *     find lots row by (auction, lot_no)
 *     check that file's SERVER PRICE (col I) == lots.price
 *     report:
 *       - match           file's SERVER PRICE == DB price
 *       - server_diff     file's SERVER PRICE != DB price
 *       - server_empty    file's SERVER PRICE is blank
 *       - missing_server  lot is in file but not in DB
 *     END
 *   FOR EACH lot in the DB for the picked auction not seen in the file
 *     report: missing_file
 *     END
 *
 * Column G (PRICE — the auctioneer's manual figure) is NOT validated.
 * It's preserved in the output for context (so the operator can see
 * both numbers side-by-side) but it doesn't participate in the
 * match/mismatch decision. The macro's `checkRate` was effectively
 * doing this too — its filter only surfaced rows with EMPTY G/H/I,
 * not rows where G != I. We've removed that quirk in favour of an
 * explicit, predictable rule.
 *
 * Public API:
