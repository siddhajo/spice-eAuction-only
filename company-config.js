...
const DEFAULTS = [
  ...
  { key: 'dispatched_through', value: '',            category: 'invoice',   label: 'Dispatched Through',       type: 'text' },
  ...
  { key: 'business_state',  value: 'BOTH',           category: 'mode',      label: 'Business State',           type: 'select' },
  ...
  { key: 'tally_inv_prefix',      value: 'ISP/',           category: 'tally', label: 'Voucher Prefix',                 type: 'text' },
...
const CATEGORIES = {
  ...
  tally:      { order: 13, title: 'To Tally',             icon: '📤', description: 'Configure all settings for the Tally XML export. Ledger names here MUST match what exists in your Tally company; if a ledger is missing or misspelled, Tally will reject the import.' },
};

// Keys that have been removed from the schema. We delete them on every
// startup so old DBs don't carry around dead config rows. Idempotent.
const DEPRECATED_KEYS = [
  // Sister concern + ASP-specific legacy
  's_logo','s_company','s_short_name','s_address1','s_address2','s_phone',
  's_email','s_gstin','s_cin','s_pan','s_fssai','s_sbl','s_place','s_pin',
  's_state','s_st_code','s_mobile',
  // Invoice / flag obsoletes
  'inv_prefix_sister',
  'dispatched_through_isp','dispatched_through_asp',
  'flag_sister','flag_tnpa','flag_rtds_inv','flag_disc_gst','flag_eway',
  // Profit ratio splits — no longer used (e-Trade gone)
  'asp_profit_pooler','asp_profit_dealer','isp_profit_pooler','isp_profit_dealer',
  'asp_profit','isp_profit',
  // Tally — ASP / sister / dispatch-from obsoletes
  'tally_ainv_prefix','tally_state_code_amazing','tally_amazing_mode',
  'tally_optional','tally_dispatch_from','tally_dispatch_company',
  'tally_dispatch_address','tally_dispatch_place','tally_dispatch_pin',
...
function initCompanySettings(db) {
  ...
  // Drop deprecated/obsolete keys (sister concern, ASP voucher prefix, etc.)
  const delStmt = db.prepare('DELETE FROM company_settings WHERE key = ?');
  const dropDeprecated = db.transaction(() => {
    for (const k of DEPRECATED_KEYS) delStmt.run(k);
  });
  dropDeprecated();

  // Drop legacy preset tables — preset switching has been removed.
  try { db.exec('DROP TABLE IF EXISTS company_presets'); } catch (_) {}
  try { db.exec('DROP TABLE IF EXISTS company_preset_meta'); } catch (_) {}

  // business_mode value 'e-Trade' is no longer supported. Coerce to 'e-Auction'.
  try {
    db.prepare("UPDATE company_settings SET value = 'e-Auction' WHERE key = 'business_mode' AND value <> 'e-Auction'").run();
  } catch (_) {}

  console.log('Company settings ready (%d defaults, %d deprecated keys cleared)',
    DEFAULTS.length, DEPRECATED_KEYS.length);
}
