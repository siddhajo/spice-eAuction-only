...
<HSNLEDGERSOURCE>${xe(LDR_Transport)}</HSNLEDGERSOURCE>
...
<HSNSOURCETYPE>Ledger</HSNSOURCETYPE>
<HSNLEDGERSOURCE>${xe(LDR_Insurance)}</HSNLEDGERSOURCE>
<GSTOVRDNTYPEOFSUPPLY>Services</GSTOVRDNTYPEOFSUPPLY>
<GSTRATEINFERAPPLICABILITY>As per Masters/Company</GSTRATEINFERAPPLICABILITY>
...
<GSTSOURCETYPE>Ledger</GSTSOURCETYPE>
<HSNSOURCETYPE>Ledger</HSNSOURCETYPE>
<GSTLEDGERSOURCE>${xe(ledger)}</GSTLEDGERSOURCE>
<HSNLEDGERSOURCE>${xe(ledger)}</HSNLEDGERSOURCE>
<GSTOVRDNSTOREDNATURE>${nature}</GSTOVRDNSTOREDNATURE>
<GSTOVRDNTYPEOFSUPPLY>Goods</GSTOVRDNTYPEOFSUPPLY>
<GSTHSNNAME>${xe(HSN_Card)}</GSTHSNNAME>
...
<NUMBERINGSTYLE>Manual</NUMBERINGSTYLE>
<PERSISTEDVIEW>Invoice Voucher View</PERSISTEDVIEW>
<VOUCHERTYPENAME>${xe(vchType)}</VOUCHERTYPENAME>
<VCHSTATUSVOUCHERTYPE>${xe(vchType)}</VCHSTATUSVOUCHERTYPE>
...
<GSTOVRDNISREVCHARGEAPPL>&#4; Not Applicable</GSTOVRDNISREVCHARGEAPPL>
<GSTSOURCETYPE>Ledger</GSTSOURCETYPE>
<GSTLEDGERSOURCE>${xe(discountLdr)}</GSTLEDGERSOURCE>
<HSNSOURCETYPE>Ledger</HSNSOURCETYPE>
...
function buildDebitNoteRows(db, auctionId, cfg) {
  ...
  // (the trade number) directly. We filter by `ano` (the stable join key)
  // rather than by date: DN dates have historically drifted from the
  // auction date (an old rule set them to trade.date + 1; they are now
