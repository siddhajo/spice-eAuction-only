...
// ── Logo upload/delete ────────────────────────────────────────
// One logo: 'ispl' (the only company). File saved to public/ so the
// PDF generator can read it at runtime, and served to the browser at
// GET /logo-ispl.png.
const LOGO_FILES = {
...
app.post('/api/company-settings/logo/:which', requireSettingsWrite, upload.single('file'), (req, res) => {
  ...
  if (!LOGO_FILES[which]) return res.status(400).json({ error: 'Invalid logo type (use ispl)' });
...
app.post('/api/invoices/generate/:auctionId', requireInvoiceWrite, (req, res) => {
  ...
  // Stamp invoice state from buyer/sale type — ISP path only.
  const invoiceState = auction.state || cfg.business_state || '';
...
app.get('/api/invoices/eligible-buyers/:auctionId', requireView, (req, res) => {
  ...
  // Eligibility: a lot is eligible if it has no `invo` set yet.
  const eligibleExpr = `(l.invo IS NULL OR l.invo = '')`;
...
app.post('/api/invoices/generate-all/:auctionId', requireInvoiceWrite, (req, res) => {
  ...
  // default sale matches (or whose lots already have that sale assigned)
  // are included. Un-invoiced means `invo` is empty.
  const uninvoicedExpr = `(l.invo IS NULL OR l.invo = '')`;
  const params = [req.params.auctionId];
  let saleClause = '';
  if (saleType) {
    saleClause = ` AND (COALESCE(NULLIF(l.sale,''), b.sale, 'L') = ?)`;
...
// grid correctly. Buyer is always the primary company (ISP).
function enrichPurchaseForPDF(invoice, cfg, db, auctionId) {
  ...
  // Buyer block — ISP primary company (sister/ASP context removed).
  if (!invoice.buyer) {
    invoice.buyer = {
...
function getDispatchPin(db) {
  ...
  // Use the Kerala office PIN (cardamom belt) by default; fall back to
  // a hard-coded Nedumkandam PIN if nothing is configured.
  return String(cfg.kl_pin || '685553').trim();
}
